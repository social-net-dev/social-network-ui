...
| `pnpm gen:api` | Generate API hooks from OpenAPI spec (Orval) |
...
- Checks `.ts`, `.tsx` files
- Enforces no-restricted-imports rule: avoid manual API services, use generated hooks
...
- Leverage generated API hooks from `@/lib/api/generated` (Orval)
...
│   ├── api/
│   │   ├── services/    # Manual API services (prefer generated)
│   │   ├── types/       # API types
...
```bash
pnpm gen:api  # Generates hooks from OpenAPI spec
```

**Generated hooks** in `@/lib/api/generated`:

```typescript
import { useGetUserQuery } from '@/lib/api/generated/hooks';
...
### Manual API Services (Legacy)

Located in `src/features/[feature]/services/`. Prefer generated hooks.
