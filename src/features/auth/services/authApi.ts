...
export const authApi = {
    ...
    try {
      ...
      const user_id =
        (resData?.user_id ?? resData?.id ?? inner?.user_id ?? inner?.id) != null
          ? String(
              resData?.user_id ?? resData?.id ?? inner?.user_id ?? inner?.id,
            )
          : "";
      const message =
        (resData?.message ?? inner?.message) != null
          ? String(resData?.message ?? inner?.message)
          : "OTP sent";
      return { user_id, message } as RegisterResponse;
    } catch (error: any) {
      const detail =
        error?.response?.data?.detail || error?.message || "Đăng ký thất bại";
  ...
  login: async (data: LoginFormData): Promise<AuthResponse> => {
    try {
      // etechs-middleware: Form-urlencoded body
      ...
      const user: User = {
        ...
        displayName:
          raw?.display_name != null ? String(raw.display_name) : undefined,
        firstName: (raw?.display_name as string)?.split?.(" ")?.[0] ?? "",
        lastName:
          (raw?.display_name as string)?.split?.(" ")?.slice(1)?.join?.(" ") ??
          "",
