...
import { 
  ...
  BookOpen, Heart, Terminal, Globe, Trash2
} from 'lucide-react'
import { 
  EditBasicInfoDialog, 
...
export function PersonalProfilePage() {
  ...
  useEffect(() => {
    if (profile?.personalInfo?.projects) {
      setLocalProjects(profile.personalInfo.projects)
    }
  }, [profile])
  ...
  const handleDeleteProject = (projectId: string) => {
    ...
    const newProjects = localProjects.filter(p => p.id !== projectId)
    setLocalProjects(newProjects)
    saveToBackend({ projects: newProjects })
  }
    ...
    <div className="max-w-[1200px] mx-auto px-4 py-8 space-y-8 animate-fadeIn">
          ...
          <div className="space-y-8">
            ...
            <Card className="rounded-3xl border border-border shadow-xl overflow-hidden bg-gradient-to-br from-etechs-primary/10 to-transparent">
                <CardContent className="p-8 flex flex-col justify-center text-center">
                    ...
                    <Button variant="outline" className="rounded-2xl font-bold border-etechs-primary/30 hover:bg-etechs-primary/10">Cập nhật ngay</Button>
                </CardContent>
            </Card>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    {profile.personalInfo?.school ? (
                        <div className="flex items-center gap-4 bg-muted/30 p-5 rounded-2xl border-l-4 border-etechs-primary">
                            ...
                            <div>
                                ...
                                <p className="text-muted-foreground text-xs font-medium">{profile.personalInfo.major} • {profile.personalInfo.graduationYear}</p>
                            </div>
                        </div>
                    ) : (
                ...
                <Card className="rounded-3xl border border-border shadow-xl overflow-hidden hover:shadow-2xl transition-all duration-300">
                    ...
                    <div className="flex flex-wrap gap-2">
                        {personalInterests.length > 0 ? personalInterests.map((interest) => {
            ...
            <Card className="rounded-3xl border border-border shadow-xl overflow-hidden">
                ...
                <CardContent className="pt-6">
                    <div className="flex flex-wrap gap-3">
                        {academicInterests.length > 0 ? academicInterests.map((tag) => (
        ...
        <section className="mt-12 space-y-8">
          ...
          <div className="grid grid-cols-1 gap-8">
            {localProjects.length > 0 ? localProjects.map(project => (
                ...
                <div className="bg-muted/20 border-2 border-dashed border-border rounded-[3rem] p-20 text-center space-y-4 animate-pulse">
                    ...
                    <Button onClick={() => document.getElementById('add-project-btn')?.click()} variant="outline" className="rounded-2xl font-bold">Thêm ngay</Button>
                </div>
            )}
          </div>
        </section>
