    ...
    <SidebarProvider>
      ...
      <SidebarInset>
        <header className="flex h-16 shrink-0 items-center gap-2 transition-[width,height] ease-linear group-has-data-[collapsible=icon]/sidebar-wrapper:h-12 border-b border-border/50 bg-background/80 backdrop-blur-md sticky top-0 z-30">
          <div className="flex w-full items-center justify-between gap-2 px-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button variant="ghost" size="icon" onClick={toggleTheme} className="h-9 w-9 rounded-xl hover:bg-primary/10 transition-colors-300" title={theme === 'light' ? 'Chuyển sang chế độ tối' : 'Chuyển sang chế độ sáng'}>
        ...
        <div className="flex flex-1 flex-col gap-4 p-4 md:p-6 pt-0">
          <div className="animate-fadeIn mt-6">{children}</div>
        </div>
        <Toaster
          position="top-right"
          richColors
