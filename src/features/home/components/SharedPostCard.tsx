    ...
    <div className="bg-muted/30 dark:bg-muted/10 rounded-xl p-4 border border-border/80 mb-4 animate-fadeIn">
      ...
      {images.length > 0 && (
        <div className={cn('grid gap-2', images.length === 1 ? 'grid-cols-1' : 'grid-cols-2')}>
          {loadingImages ? <div className="col-span-full h-32 bg-muted/30 rounded-lg animate-pulse" /> : blobUrls.map((url, i) => <img key={i} src={url} className={cn('w-full object-cover rounded-lg border border-border/30', images.length === 1 ? 'h-48' : 'h-32')} alt="Shared content" />)}
        </div>
      )}
    </div>
  );
